...
export = class extends FgoModule {
    require = [new mst().id];
    constructor() {
        super(`np-details-cache`, {});
    }

    path : string;
    private log = new componentLog(`F/GO NP details`);
    ...
    private process = (collectionNo : number) => {
        ...
        return new EmbedRenderer(NA, JP, _comp).treasureDeviceEmbed(collectionNo);
    }

    async initialize() {
        let path = mkdtempSync(join(tmpdir(), `pepper_${process.getuid()}_np_`), "utf-8");
        this.path = path;
        ...
        let _ = await _mst.JP.mstSvt.find({ $or: [{ type: SvtType.HEROINE }, { type: SvtType.NORMAL }] }).select('collectionNo name');
            ...
            queue.add(
                async () => {
                    try {
                        ...
                        this.log.success(`Processed NP of servant ${collectionNo}. ${
                            c.bgBlue.yellowBright(name)
                        }`);
                        this.push(collectionNo, e.map(np => np.toJSON()));
                    } catch (e) {
                        this.log.error(`Error processing NP of servant ${collectionNo}.\n${e.stack}`)
                    };
                }
            )
        };
